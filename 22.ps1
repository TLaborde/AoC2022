    ...
    else
    {
        ...
        for ($i = 0; $i -lt $move; $i++)
        {
            ...
            if (!$map[$nextStep[0]] -or !$map[$nextStep[0]][$nextStep[1]])
            {
                ...
                {
                    #from right, arrive to back from right
                    $nextstep = @((199 - ($nextStep[1] % 50)), 99)
                    $newDirection = 2
                }
                ...
                elseif ($me.position[1] -eq 99 -and $nextStep[1] -eq 100)
                {
                    ...
                    {
                        # from front to right
                        $nextstep = @(49, ($me.position[0] + 50))
                        $newDirection = 3
                    }
                    elseif ($me.position[0] -ge 150)
                    {
                        # from back to right
                        $nextstep = @(0, 149 - ($me.position[0] % 50))
                        $newDirection = 1
                    ...
                    else
                    {
                        ...
                        $nextstep = @((49 - ($me.position[0] % 50) ), 149)
                        $newDirection = 2
                    }
                }
                elseif ($nextStep[0] -eq 200 -and $nextStep[1] -ge 50)
                {
                    # from back to top
                    $nextStep[0] = 0
                }
                elseif ($nextStep[0] -eq 200 -and $nextStep[1] -lt 50)
                {
                    # from left to top
                ...
                elseif ($nextstep[0] -eq -1)
                {
                    # from left to front
                    $nextStep = @(99 - ($nextStep[0] % 50), 50)
                    $newDirection = 0
                }
                elseif ($me.position[0] -eq 150 -and $nextStep[0] -eq 149)
                {
                    # from left to bottom
                    $nextStep = @(100 + $nextStep[1], 50)
                    $newDirection = 0
                }
                elseif ($me.position[1] -eq 50 -and $me.position[0] -ge 100)
                {
                    # from bottom to left
                    $nextStep = @(150, ($nextStep[0] % 50))
                    $newDirection = 1
                }
                elseif ($me.position[1] -eq 50 -and $me.position[0] -ge 50 -and $me.position[0] -lt 100)
                {
                    # from front to left
                    $nextStep = @((199 - ($nextStep[0] % 50)), 0)
                    $newDirection = 0
                }
                elseif ($me.position[1] -eq 50 -and - $nextStep[1] -eq 49 -and $me.position[0] -lt 50)
                {
                    # from top to left
                    $nextStep = @(199, (50 - ($nextStep[0] % 50)))
                    $newDirection = 3
                }
            }
            if ($map[$nextStep[0]][$nextStep[1]] -eq 'free')
